...
export const inspectionFormAPI = {
  ...
  downloadPdf: async (id,username) => {
    try {
      const response = await fetch(`http://localhost:8080/api/inspection-forms/${id}/pdf/${username}`, {
        method: 'GET',
        headers: {
  ...
  sendEmailWithPdf: async (id, emailData) => {
    try {
      const response = await fetch(`http://localhost:8080/api/inspection-forms/${id}/email-pdf`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
